...
public class ShopArr {
	...
	private void intro(Scanner input) {
		...
		for(int i = 0; i < cheese.length; i++){
			System.out.print("Ener the amount of " + cheese[i].getName() + ": ");
	...
	private void itemizedList(){
		...
		for (int i = 0; i < cheese.length; i++){      
			if ((amt = cheese[i].getAmount()) > 0)
				System.out.println(amt + " lbs of " + cheese[i].getName() + " @ $" + cheese[i].getPrice() + " = " + cheese[i].getAmount()*cheese[i].getPrice());
		}
	}
